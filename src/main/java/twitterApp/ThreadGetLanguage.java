...
public class ThreadGetLanguage extends Thread {
	private JTextPane txt;

	public ThreadGetLanguage(JTextPane txt) {
		this.txt = txt;
	}
	...
	public void run() {
		...
		try {
			...
			HttpURLConnection con = (HttpURLConnection) (new URL(
					"http://localhost:5984/twitter_aburam/_design/application/_view/getLanguage?group=true"))
					.openConnection();
			...
			try {
				...
				for (Map.Entry<String, Integer> entree : result.entrySet()) {
					...
					try {
						Locale l = new Locale(entree.getKey());
